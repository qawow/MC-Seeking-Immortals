...
CANONICAL_COUNTS = {
    ...
    "NPC": 179, "REALM": 20, "ZONE": 79, "BOSS": 28, "STATUS": 22,
...
PROGRAM_RULES = (
    ...
    ("magic_box", ("金属方盒", "黑色长匣", "黑匣", "玉盒", "木盒", "方盒", "玉匣", "匣盖",
                   "刺目红芒先从盒中")),
    ("magic_needle", ("白濛濛的牛毛般细针", "纤细如丝的细针", "乌黑细针", "黄色细针",
                      "牛毛般金针", "金针", "银针", "灵针", "飞针")),
    ...
    ("seal_stamp", ("白光中的大印", "白玉大印", "血色巨印", "方印章",
                    "法印一闪", "大印一压", "大印落下", "法印一", "玉印", "宝印",
                    "印诀", "金印", "血印", "印玺")),
    ("command_token", ("黑色令牌", "长老令牌", "禁制令牌", "令牌上血光", "手中令牌",
                       "金色令牌", "从令牌一面", "金银令牌", "玉牌",
                       "数块漆黑的木牌", "单手抓着铁牌", "轻轻一晃自己的玉佩",
                       "绿牌翠芒一闪")),
    ("magic_mask", ("一张青色面具",)),
    ...
    ("scripture_glyph", ("经文", "书卷", "典籍", "墨字", "金文", "古字", "符字", "真言",
                         "银灿灿「儒」字", "数字瞬间就由「二」字，变成了「三」",
                         "一个「一」字浮现在")),
    ("wing_fan", ("双翼", "羽翼", "翅膀", "羽翅", "风雷翅", "血红晶翅")),
    ("spear_spike", ("金色巨钉", "长矛", "冰矛", "冰锥", "地刺", "骨刺", "蓝色细刺",
    ...
    ("chain_net", ("锁链", "电网", "锁链网", "铁链", "灵链", "缚仙索", "缠丝成网",
                   ...
                   "五股青光从指尖处激射而出", "一张青色大网", "无数绿丝包裹",
                   "绿丝包裹着吸入")),
    ("beam_lance", ("光柱", "光束", "剑虹", "剑光", "五股青光从指尖处激射而出")),
    ("channel_stream", ("从上面喷出了一丝黑线", "顿时一丝青光喷出")),
    ("barrier_plane", ("晶莹异常的障壁",)),
    ("projectile_swarm", ("飞射", "箭雨", "无数", "密密麻麻")),
    ("body_aura", ("鳞片", "金身", "铠甲", "护体")),
    ("spirit_avatar", ("法相", "鬼影", "骷髅", "化身", "人形", "女子", "高大人影", "五具骨架",
                       "高大的黑色巨人")),
    ("ground_field", ("地面", "大地", "领域", "地网", "丈许深大坑",
                      "大坑表面就变得平整光滑")),
...
def program_primitive_path(primitive: str, text: str, fallback: str) -> str:
    if primitive in {"confucian_sage", "rune_stele", "command_token"}:
        return "STATIC"
    if primitive == "formation_rod":
        if "缓缓拉长" in text:
            return "RISE"
        if "扭动不停" in text:
            return "ORBIT"
        return "STATIC"
    if primitive == "talisman_brush" and "五色大笔" in text:
        return "STATIC"
    if primitive == "chain_net" and any(token in text for token in (
            "五股青光从指尖处激射而出", "一张青色大网")):
        return "EXPAND"
    if primitive == "chain_net" and "无数绿丝包裹" in text:
        return "CONVERGE"
    if primitive == "chain_net" and "绿丝包裹着吸入" in text:
        return "CONVERGE"
    if primitive == "beam_lance" and "五股青光从指尖处激射而出" in text:
        return "DIRECT"
    if primitive == "beam_lance" and "数十道光柱发出惊人轰鸣的拔地而起" in text:
        return "RISE"
    if primitive == "rune_pillar" and "数十道光柱发出惊人轰鸣的拔地而起" in text:
        return "STATIC"
    if primitive == "channel_stream" and any(token in text for token in (
            "从上面喷出了一丝黑线", "顿时一丝青光喷出")):
        return "DIRECT"
    if primitive == "beast_phantom" and any(token in text for token in (
            "迷你绿色雷兽虚影", "雷兽精魂")):
        return "TRACK"
    if primitive == "barrier_plane" and "晶莹异常的障壁" in text:
        return "STATIC"
    ...
    if primitive in {"magic_screen", "magic_page"}:
        return "STATIC"
    if primitive == "scripture_glyph" and "无数金篆文从书页上狂涌而出" in text:
        return "DIRECT"
    if primitive == "channel_stream" and "一道蓝光打出……就任由蓝光没入其内" in text:
        return "DIRECT"
    if primitive == "light_curtain" and "五色光罩从大殿底部浮现" in text:
        return "RISE"
    if primitive == "orb_projectile" and "化为一团巨型光球" in text:
        return "STATIC"
    if primitive == "rune_orbit" and "整座大殿无数蓝色符文" in text:
        return "SCATTER"
    if primitive == "rune_orbit" and "巨大的宫殿虚影一闪即逝" in text:
        return "ORBIT"
    if primitive == "channel_stream" and "幻化成了一座布置典雅的精美殿堂" in text:
        return "DIRECT"
    if primitive == "projectile_swarm" and "金色宫殿也寸寸崩碎" in text:
        return "SCATTER"
    ...
    if primitive == "magic_needle":
        if any(token in text for token in ("围绕", "盘旋")):
            return "ORBIT"
        if "纷纷坠落" in text:
            return "FALL"
        if "弹射而飞" in text:
            return "SCATTER"
        if any(token in text for token in ("插满", "轻颤", "紫冰包裹")):
            return "STATIC"
        if any(token in text for token in (
                "飞进了玉匣", "激射", "幻扎", "扎进", "没入", "弹入", "一扎")):
            return "DIRECT"
        if "飞出" in text:
            return "DIRECT"
        return fallback
...
_EXACT_LOCAL_PALETTE_TERMS = {
    "confucian_sage": frozenset({"十几丈高的白色人影", "儒家圣像"}),
    "formation_rod": frozenset({
        "金黄色木棍般的东西", "黄色晶丝从木棍上腾起", "十二根青色木棍",
    }),
    "rune_stele": frozenset({"石碑微颤之下，通体灵光闪烁"}),
    ...
    "magic_screen": frozenset({
        "屏风上顿时浮现出一层白色晶芒", "黑色门扉赫然从屏风上飞射而出",
        "单手抓着铁牌走到屏风前", "在屏风上一处丝毫标记没有的灵地上",
    }),
    "magic_page": frozenset({
        "一个玉盒内的金色书页", "书页上的光芒缓缓收敛",
        "冲那黑色书页一招手", "精血在书页上飞快的写",
        "一道蓝光打出……就任由蓝光没入其内",
        "无数金篆文从书页上狂涌而出",
    }),
...
_EXACT_LOCAL_PALETTE_FALLBACKS = {
    "confucian_sage": "qi", "formation_rod": "earth", "rune_stele": "qi",
    ...
    "fortress_wall": "earth", "magic_gate": "qi", "magic_screen": "qi",
    "magic_page": "qi",
...
def program_palette_source(text: str, primitive: str, matched_terms: list[str]) -> str:
    ...
    if primitive == "magic_needle":
        # Needle colours frequently follow the noun (red flight lines, green glints,
        # purple ice), while later blood/talisman effects must not recolour the metal.
        return text[max(0, index - 24):min(len(text), index + len(term) + 28)]
...
_FIGURE_PRIMITIVES = {
    ...
    "magic_chariot", "magic_hall", "fortress_wall", "magic_gate", "magic_screen",
    "magic_page",
...
def program_primitives(text: str, base_shape: str,
    ...
    if profile_id == "technique_1134":
        allowed = {"confucian_sage", "eye_gaze", "talisman_brush", "scripture_glyph"}
        selected = [primitive for primitive in selected if primitive in allowed]
        selected = ["confucian_sage"] + [
            primitive for primitive in selected if primitive != "confucian_sage"
        ]
        evidence.append("forced:continuous_confucian_sage")
    if profile_id == "technique_910":
        selected = [primitive for primitive in selected if primitive != "projectile_swarm"]
        evidence.append("forced:single_green_net_and_rods")
    if profile_id == "technique_670" and any(token in text for token in (
            "迷你绿色雷兽虚影", "雷兽精魂")):
        selected = [primitive for primitive in selected if primitive != "projectile_swarm"]
        evidence.append("forced:bound_thunder_beast_soul")
    if profile_id == "technique_605":
        selected = [primitive for primitive in selected if primitive != "ground_field"]
        evidence.append("forced:map_not_ground")
    if "刺目红芒先从盒中爆发" in text and "impact_arcs" not in selected:
        selected.append("impact_arcs")
        evidence.append("forced:box_light_burst")
    if "magic_needle" in selected and "紫冰包裹" in text and "ice_prison" not in selected:
        selected.append("ice_prison")
        evidence.append("forced:needle_ice_prison")
    ...
    if "magic_needle" in selected and any(token in text for token in (
            "仿佛万千飞针", "金针般细小飞剑", "如同钢针般")):
        selected.remove("magic_needle")
        evidence.append("forced:needle_simile_excluded")
    ...
    if profile_id == "technique_859" and "巨大的宫殿虚影一闪即逝" in text:
        selected = ["magic_hall", "rune_orbit"]
        evidence.append("forced:nine_palace_apparition")
    elif (profile_id == "technique_962"
          and "整座大殿……徐徐的从土中直接拔地而起" in text):
        selected = ["magic_hall", "light_curtain", "orb_projectile"]
        evidence.append("forced:rising_city_hall")
    elif profile_id == "technique_1104" and "整座大殿无数蓝色符文飘舞而出" in text:
        selected = ["magic_hall", "rune_orbit"]
        evidence.append("forced:rune_hall_retreat")
    elif profile_id == "technique_1178" and "幻化成了一座布置典雅的精美殿堂" in text:
        selected = ["magic_hall", "channel_stream"]
        evidence.append("forced:domain_hall_materialization")
    ...
    if profile_id == "technique_605" and any(token in text for token in (
            "单手抓着铁牌走到屏风前", "在屏风上一处丝毫标记没有的灵地上")):
        selected = ["magic_screen", "command_token", "scripture_glyph", "channel_stream"]
        evidence.append("forced:marked_map_screen")
    elif profile_id == "technique_1478" and "屏风上顿时浮现出一层白色晶芒" in text:
        selected = ["magic_screen"]
        evidence.append("forced:living_landscape_screen")
    elif profile_id == "technique_031" and "一个玉盒内的金色书页" in text:
        selected = ["magic_page", "magic_box", "scripture_glyph"]
        evidence.append("forced:boxed_golden_page")
    elif profile_id == "technique_035" and "书页上的光芒缓缓收敛" in text:
        selected = ["magic_page", "scripture_glyph"]
        evidence.append("forced:glowing_scripture_page")
    elif profile_id == "technique_399" and "冲那黑色书页一招手" in text:
        selected = ["magic_page", "orb_projectile"]
        evidence.append("forced:blood_before_black_page")
    elif profile_id == "technique_399" and "精血在书页上飞快的写" in text:
        selected = ["magic_page", "ghost_head", "scripture_glyph", "fire_plume"]
        evidence.append("forced:blood_oath_page")
    elif (profile_id == "technique_399"
          and "一道蓝光打出……就任由蓝光没入其内" in text):
        selected = ["magic_page", "channel_stream"]
        evidence.append("forced:sealed_black_page")
    elif profile_id == "technique_1215" and "无数金篆文从书页上狂涌而出" in text:
        selected = ["magic_page", "scripture_glyph"]
        evidence.append("forced:scripture_page_outflow")
    elif profile_id == "technique_1215" and "将经文化为点点灵光" in text:
        selected = ["scripture_glyph"]
        evidence.append("forced:scripture_dissolve")
    if profile_id == "technique_1241" and "一截截高大城墙和一座座形态各异的建筑一落而下" in text:
        ...
        selected = ["magic_screen", "magic_gate"]
    ...
    if swordish and "giant_sword" not in selected:
        ...
        keep_extra = {"lotus_mandala", "sword_rain", "projectile_swarm", "fire_plume",
                      ...
                      "bell_chime", "magic_whip", "seal_stamp"}
...
def make_visual_program(profile: dict[str, Any], raw: dict[str, Any],
    ...
    for source_index, source in enumerate(sources):
        ...
        if profile_id == "technique_1134" and "十几丈高的白色人影" in source:
            radius = max(radius, 2.2)
            height = max(height, 3.0)
        if profile_id == "technique_1178" and "布置典雅的精美殿堂" in source:
            radius = max(radius, 3.2)
            length = max(length, 2.2)
            height = max(height, 1.7)
        if "magic_screen" in primitive_ids:
            radius = max(radius, 2.2)
        ...
        for primitive_index, primitive in enumerate(primitive_ids):
            ...
            if (profile_id == "technique_859"
                    and "巨大的宫殿虚影一闪即逝" in source):
                ...
                if primitive == "magic_hall":
                    ...
                    primary_key = secondary_key = "qi"
            elif (profile_id == "technique_1104"
                  and "整座大殿无数蓝色符文飘舞而出" in source):
                if primitive == "magic_hall":
                    primary_key, secondary_key = "water", "qi"
                else:
                    primary_key = secondary_key = "water"
            elif (profile_id == "technique_1178"
                  and "幻化成了一座布置典雅的精美殿堂" in source):
                primary_key = secondary_key = "qi"
            elif (profile_id == "technique_1451"
                  and "金色宫殿也寸寸崩碎" in source):
                if primitive == "magic_hall":
            ...
            elif primitive == "magic_screen":
                if profile_id == "technique_1478":
                    primary_key, secondary_key = "qi", "water"
                else:
                    primary_key, secondary_key = "earth", "metal"
            elif primitive == "magic_page":
                if profile_id == "technique_031":
                    primary_key = secondary_key = "metal"
                elif profile_id == "technique_399":
                    primary_key = "yin"
                    secondary_key = "water" if "一道蓝光打出" in source else "fire"
                else:
                    primary_key, secondary_key = "metal", "qi"
            elif primitive == "scripture_glyph" and profile_id == "technique_035":
                primary_key, secondary_key = "metal", "qi"
            elif primitive == "scripture_glyph" and profile_id == "technique_399":
                primary_key, secondary_key = "fire", "yin"
            elif primitive == "scripture_glyph" and profile_id == "technique_1215":
                if "无数金篆文" in source:
                    primary_key, secondary_key = "metal", "qi"
                else:
                    primary_key = secondary_key = "qi"
            elif (primitive == "orb_projectile" and profile_id == "technique_399"
                  and "一团精血脱口喷出" in source):
                primary_key = secondary_key = "fire"
            elif primitive == "ghost_head" and profile_id == "technique_399":
                primary_key, secondary_key = "fire", "yin"
            elif primitive == "fire_plume" and profile_id == "technique_399":
                primary_key, secondary_key = "yin", "fire"
            elif primitive == "channel_stream" and profile_id == "technique_399":
                primary_key = secondary_key = "water"
                ...
                primary_key, secondary_key = "wood", "qi"
            elif primitive == "rune_stele":
                primary_key, secondary_key = "qi", "thunder"
            elif primitive == "magic_needle":
                if profile_id == "spirit_needle":
                    primary_key = secondary_key = "qi"
                elif profile_id == "technique_068":
                    primary_key = secondary_key = "fire"
                elif profile_id == "technique_291":
                    primary_key, secondary_key = "yin", "wood"
                elif profile_id == "technique_501":
                    primary_key, secondary_key = "qi", "water"
                elif profile_id == "technique_505":
                    primary_key, secondary_key = "earth", "metal"
                elif profile_id == "technique_1289":
                    primary_key = secondary_key = "wood"
                else:
                    primary_key = secondary_key = "metal"
            elif primitive == "seal_stamp" and profile_id == "technique_1256":
                primary_key = secondary_key = "fire"
            elif primitive == "seal_stamp" and profile_id == "technique_1401":
                primary_key, secondary_key = "earth", "qi"
            elif primitive == "seal_stamp" and profile_id in {"technique_141", "technique_599"}:
                primary_key = secondary_key = "qi"
            elif primitive == "command_token" and profile_id == "technique_098":
                primary_key, secondary_key = "yin", "wood"
            elif primitive == "command_token" and profile_id == "technique_605":
                if "铁牌" in source:
                    primary_key = secondary_key = "metal"
                else:
                    primary_key, secondary_key = "qi", "wood"
            elif primitive == "command_token" and profile_id == "technique_670":
                primary_key = secondary_key = "wood"
            elif primitive == "talisman_brush" and profile_id == "technique_1134":
                primary_key, secondary_key = "qi", "thunder"
            elif primitive == "scripture_glyph" and profile_id == "technique_1134":
                primary_key = secondary_key = "metal"
            elif primitive == "eye_gaze" and profile_id == "technique_1134":
                primary_key = secondary_key = "metal"
            elif primitive == "chain_net" and profile_id == "technique_910":
                primary_key, secondary_key = "wood", "qi"
            elif primitive == "beam_lance" and profile_id == "technique_910":
                primary_key = secondary_key = "wood"
            elif primitive == "chain_net" and profile_id == "technique_670":
                primary_key = secondary_key = "wood"
            elif primitive == "beast_phantom" and profile_id == "technique_670":
                primary_key, secondary_key = "wood", "soul"
            elif primitive == "channel_stream" and profile_id == "technique_605":
                primary_key = secondary_key = "yin" if "黑线" in source else "wood"
            elif primitive == "scripture_glyph" and profile_id == "technique_605":
                primary_key = secondary_key = "metal" if "银色光点" in source else "wood"
            elif primitive == "ground_field" and profile_id == "technique_340":
                primary_key = secondary_key = "earth"
            elif primitive in {"beam_lance", "rune_pillar", "halo_ring", "barrier_plane"} \
                    and profile_id == "technique_457":
                primary_key = secondary_key = "qi"
            elif (primitive == "rune_orbit" and profile_id == "technique_1493"
                    and "木棍" in source):
                primary_key, secondary_key = "earth", "metal"
            elif primitive == "chain_net" and profile_id == "technique_1330":
                primary_key = secondary_key = "wood"
            elif primitive == "giant_sword":
                if "石剑" in source:
                    primary_key = "earth"
            ...
            preserve_local_gradient = (
                ...
                or primitive in {"fortress_wall", "magic_gate", "magic_screen", "magic_page"}
            )
            ...
            if primitive in {"confucian_sage", "rune_stele", "talisman_brush",
                             "scripture_glyph", "barrier_plane", "channel_stream"} \
                    and profile_id in {"technique_1134", "technique_457", "technique_605"}:
                copies = 1
            if primitive == "formation_rod":
                copies = 72 if profile_id == "technique_1493" else 12
            if primitive == "command_token" and profile_id in {
                    "technique_098", "technique_605", "technique_670"}:
                copies = 5 if profile_id == "technique_098" else 1
            if primitive == "chain_net" and profile_id in {"technique_670", "technique_910"}:
                copies = 1
            if primitive == "beam_lance" and profile_id == "technique_910":
                copies = 5
            if primitive == "beast_phantom" and profile_id == "technique_670":
                copies = 1
            if primitive == "ground_field" and profile_id == "technique_340":
                copies = 1
            if profile_id == "technique_457":
                if primitive in {"rune_pillar", "halo_ring", "beam_lance"}:
                    copies = 12
                elif primitive in {"rune_stele", "barrier_plane"}:
                    copies = 1
            if primitive == "formation_disc" and "法盘" in source:
                copies = 1
            if primitive in {"magic_vajra", "magic_box", "magic_ruyi", "magic_hook", "magic_whip",
                             "spiked_shield", "seal_stamp"}:
                copies = 1
            if primitive == "magic_needle":
                if profile_id == "technique_068":
                    copies = 13
                elif profile_id in {"technique_291", "technique_408", "technique_505"}:
                    copies = 20
                elif profile_id == "technique_347":
                    copies = 5
                elif profile_id in {
                        "technique_495", "technique_501", "technique_606",
                        "technique_717", "technique_1017", "technique_1289"}:
                    copies = 12
            if primitive == "magic_boat":
                copies = 1
            if primitive == "ritual_altar":
            ...
            if primitive in {"magic_screen", "magic_page"}:
                copies = 1
            if primitive == "scripture_glyph" and profile_id == "technique_035":
                copies = 12
            if primitive == "scripture_glyph" and profile_id == "technique_399":
                copies = 8
            if primitive == "scripture_glyph" and profile_id == "technique_1215":
                copies = 20
            if (primitive == "spirit_avatar"
                    and "金色元婴正两只小手乱舞不已" in source):
                copies = 1
            ...
            if (profile_id == "technique_962"
                ...
                copies = 1
            if (profile_id == "technique_1104"
                    and "整座大殿无数蓝色符文飘舞而出" in source
                    and primitive == "rune_orbit"):
                copies = 20
            if (profile_id == "technique_1451"
                    and "金色宫殿也寸寸崩碎" in source
                    and primitive == "projectile_swarm"):
                copies = 20
            layer_path = program_primitive_path(primitive, source, path)
            layer_anchor = program_anchor(source, layer_path, trigger)
            hall_scene = "magic_hall" in primitive_ids
            layer_spread = (360.0 if layer_path == "ORBIT"
                            else (180.0 if hall_scene and layer_path == "SCATTER" else spread))
            layer_rotation = 360.0 if layer_path in {"ORBIT", "SPIRAL"} else rotation
            layer_motion = motion
            if primitive == "magic_hall":
                if (profile_id == "technique_1178"
                        and "幻化成了一座布置典雅的精美殿堂" in source):
                ...
                layer_motion = "MATERIALIZE"
            elif (primitive == "magic_screen" and profile_id == "technique_1478"
                  and "白色晶芒" in source):
                layer_motion = "MATERIALIZE"
            elif primitive == "scripture_glyph" and profile_id in {"technique_035", "technique_1215"}:
                layer_motion = ("DISSOLVE" if "将经文化为点点灵光" in source
                                else "MATERIALIZE")
            elif (profile_id == "technique_399"
                  and primitive in {"ghost_head", "fire_plume"}):
                layer_motion = "MATERIALIZE"
            ...
            if hall_scene:
                if profile_id == "technique_859":
                    layer_vertical_offset = 0.72 if primitive == "magic_hall" else 0.08
                elif profile_id == "technique_962" and primitive == "orb_projectile":
                    layer_vertical_offset = min(16.0, radius * height * 0.5)
                elif profile_id == "technique_1104" and primitive == "rune_orbit":
                    layer_vertical_offset = 0.65
                elif profile_id == "technique_1178" and primitive == "magic_hall":
                    layer_vertical_offset = 0.03
                elif profile_id == "technique_1451":
                    layer_vertical_offset = 0.03 if primitive == "magic_hall" else 0.85
            if primitive in {"fortress_wall", "magic_gate", "magic_screen"}:
                layer_vertical_offset = 0.03
            elif primitive == "magic_page":
                layer_vertical_offset = 0.62
            layers.append({
                ...
                "jitter": 0.06 if layer_motion in {"FLICKER", "PULSE"} else 0.02,
