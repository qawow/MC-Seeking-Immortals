#!/usr/bin/env python3
"""0.1.449: add remaining hard-missing carriers formation_flag_mid + spirit_herb_rare."""
...
NEW_ITEMS = [
    {
        "id": "formation_flag_mid",
        "category": "material",
        "rarity": "uncommon",
        "display": "中品阵旗",
        "source_files": ["formation_catalog.json", "novel_world_expansion_waves_v66.json"],
    ...
    },
    {
        ...
        "rarity": "rare",
        "display": "稀有灵草",
        "source_files": ["pills_catalog.json"],
        "source_category": "material",
    },
]
...
def make_icon(name: str, size: int = 64) -> Image.Image:
    ...
    if "flag" in name:
        d.polygon(
            [(size * 0.25, size * 0.15), (size * 0.75, size * 0.30), (size * 0.25, size * 0.45)],
            fill=base,
            outline=(255, 255, 255, 200),
        )
        d.line([(size * 0.25, size * 0.15), (size * 0.25, size * 0.85)], fill=(230, 230, 230, 255), width=3)
    else:
...
def main() -> None:
    ...
    for it in NEW_ITEMS:
        ...
        if rid in by:
            ...
            e["note"] = "Wave 0.1.449 hard-missing catalog carrier."
        else:
            idmap["entries"].append(
                {
                    ...
                    "status": "implemented",
                    "note": "Wave 0.1.449 hard-missing catalog carrier.",
                }
            )
    ...
    if "mod_version=0.1.448" in raw:
        gp.write_text(raw.replace("mod_version=0.1.448", "mod_version=0.1.449"), encoding="utf-8")
    elif "mod_version=0.1.449" not in raw:
