...
10. ~~**Patchouli 指南补页**~~（0.2.93 完成：工站运行态、交付信箱、消耗品语义、成长、宗门、秘境、JEI 七组双语页 + 过期页改写；book version 3）。
11. **美术/GeckoLib**：占位贴图替换，非阻塞，最后做。


### 物品语义与高阶工站收口（0.2.93–0.2.98）
- Patchouli 新系统补页（0.2.93）
- 空白载体铭刻 / 丹方研读 / bulk 手册（0.2.94）
- bulk 符箓施放（0.2.95）
- 阵具激活 + equipment 右键（0.2.96）
- 丹券兑换 + 别名丹药分类确认（0.2.97）
...
- 丹药命名分条效果与境界辅修分阶（0.2.102）
- 美术/GeckoLib 可落地深度：仆从动画、灵舟可见、技能图标库（0.2.103）
- 专服/双客户端多人复签基建 + 专服 Screen 侧加载修复（0.2.104→0.2.105）
- cultivation 遗留术法高层 type→可执行 effectType（0.2.106）
- 渡口物品路由 + 14 空 method 矩阵补术（0.2.107）
- 工站 id 契约 + 飞舟泊桩专属校验（0.2.108）
- 结构修复台/蓝图台可右键对接运行态（0.2.109）
- 傀儡核心炉/化形池专属校验 + 渡口费 OR 门（0.2.110）
- 软秘籍/炼器册显式授予功法与品阶（0.2.111）
- 剩余非代码：工作室级手绘 GeckoLib 精模、完整实机烟测、真人专服双客户端签字
