...
仍未完成：没有结构化 `place/need` 的作者叙事步骤按安全策略只接受精确服务端证据，目前部分只能由管理员 `prove` 或未来专用事件提供；不能把中文 `do` 文本当状态机。阴阳窟使用现有通用外/中/核心试炼壳与阴雾环境，尚未实现专属银翅夜叉实体编队、阴芝马活捕判定和窟外协作炼丹。自动测试不能替代客户端、专服和多人长时实机验收。
...
仍需真实客户端/专服/多人验证周期体感、全部入口状态的正常生存可达性、多人抢杀和受控单位归属、掉线/死亡/跨日、满背包交付及随机奖励平衡。对话 `mark_structure`/线索/守卫/逮捕等动作的逐类保真，以及 95 个详细任务步骤中 `do/place/need/fail/reward` 的结构化权威，仍是明确未完成项。
