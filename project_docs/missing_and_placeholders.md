...
供给链缺口已关闭：84 条已发布配方中原料无任何发放位的死配方由 55 条降为 0 条，造纸阶梯、符墨配方、9 家商店补货和 8 个首领掉落均已接入数据树。该结论只证明“存在获取路径”，不证明价格、掉率或正常生存路线的经济手感。

仍需明确记录的缺口：

- 制符运行时仍由 `TalismanCraftService.buildRecipes()` 硬编码 24 条配方，当前只产 5 种成品；`talisman_recipes.json` 的 24 种成品尚未被运行时读取，`recipeBlueprintCount()` 仍直接返回 24。
- 造纸阶梯和墨配方目前是普通 shapeless 工作台配方，未执行作者数据标注的 `talisman_table` 工站与境界门槛；这是规格偏差，不应在 QA 中写成已完成的制符工站闭环。
- 本批商店价格和首领掉率是按同档内容设定的手感值，尚未通过真实客户端/专服经济流程验证；高境界首领掉落仍受秘境进入门槛影响。
- 约 646 个数据驱动 bulk 物品仍待按配方、掉落、商店、任务、区域事件等真实渠道分类；“目录存在”不等于“玩家可获取”。
- QA-01 单客户端、QA-02 专服+双客户端，以及完整实机烟测仍待实际执行。权限 2 指令会改变持久状态，不能作为正常流程可达性的证明。

命令根路径、作弊/调试指令、异常状态注入、管理员变更和签字流程统一记录在 `project_docs/command_reference.md`。
...
已接入（`0.2.245`-`0.2.251`，Q-B 系列完成）：修炼/境界/功法/术法、地域/维度/秘境/结构/灵根测试、物品获取/制作/炼丹/交付、击杀/活捕/护送/遭遇以及 NPC 对话/选择/商店/拍卖/声望/规则告知事件均有结构化证明生产者和服务端实时校验；`spirit_root_test`、`zm_candle`、`tianyuan_garrison_board` 与 `hy_core` 已按规则重新分类，六个故事载体（太阳精火/五寒焰/银蝌文/龙鳞果/无字信/灰界线索）已注册。仍未完成：`contribution_stele` 与 `inner_sect_task_board` 尚无已注册方块无法成型；四项规则告知（mortal_qixuan_entry_step_1 等）、三项选择（bf_mid_contest 等）、三家商店（heifeng_sea_route 等）与 `fallen_demon_token` 拍卖无运行时内容层，对应步骤失败关闭；`wuxing_shallow_trial`/`gray_realm_border`/`heifeng_sea` 遭遇与灰界护送尚无内容层；`yin_zhi_horse` 活捕路由待 Y-B 补灵兽本体；队伍共享推进未实现；新载体掉落/配方/商店来源由作者资料驱动。阴阳窟使用现有通用外/中/核心试炼壳与阴雾环境，尚未实现专属银翅夜叉实体编队、阴芝马活捕判定和窟外协作炼丹。自动测试不能替代客户端、专服和多人长时实机验收。
...
仍需真实客户端/专服/多人验证周期体感、全部入口状态的正常生存可达性、多人抢杀和受控单位归属、掉线/死亡/跨日、满背包交付及随机奖励平衡。对话 `mark_structure` 已在 `0.2.247` 收紧为真实成型、正式启用且维度匹配的结构证明；95 个详细任务步骤的全部事件域（修炼/世界/物品/战斗/对话）已在 `0.2.245`-`0.2.251` 接入结构化权威（Q-B 系列完成）；线索/守卫/逮捕等对话世界动作的逐类保真（D-A）仍是明确未完成项。
