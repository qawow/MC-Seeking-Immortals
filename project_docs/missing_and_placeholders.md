...
自动化构建与语料索引已闭合，但尚未完成真实客户端、专服和多人烟测。已知功能深度风险包括：M04 部分炼丹配方缺少可靠定向公式入口；M05 动态价格/跨币种换算需实机回归；M07/M13 飞舟船坞结构定义未统一；M13 飞升备份恢复仍需防复制审计；M14 的 outgoingDamageMul、blocksTechnique、hidesRealm 与 M07/M15 伤害钩子尚未全部接线；M15 器灵觉醒/祭炼成长链和装备扫描需继续深化。高保真结构、专用妖兽实体与美术资产仍是明确遗留。
