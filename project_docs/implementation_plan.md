# 寻仙问道 — `0.2.244` 后续完整实现计划

> 状态：F-A（UI-01 至 UI-05）、F-B（UI-06、UI-18）、F-C1（UI-07 至 UI-11、UI-19）、F-C2（UI-12 至 UI-14、UI-24 至 UI-26）、F-C3（UI-15、UI-28）、F-D（UI-16、UI-17、UI-21、UI-23、UI-27）、F-E1（UI-20）、F-E2（UI-22）与 Q-A（95 步证明路由）已完成并验证；下一实施入口为 Q-B。本文件继续定义剩余工作的实施顺序、边界和验收门。
> 制定日期：2026-07-29。
> 最近已提交基线：`3ae9d12b refactor: 删除不可达旧界面`；Q-A 已完成并记录，下一批为 Q-B。
> 初始版本基线：`mod_version=0.2.232`；当前执行版本：`mod_version=0.2.244`。
> 网络基线：`ModNetwork.PROTOCOL_VERSION=31`。
> 最近完整自动验证：256 个测试套件、1,213 项测试通过，failure/error/skipped 均为 0；`0.2.244` 完整 Gradle 构建结果记录在本批更新记录中。
