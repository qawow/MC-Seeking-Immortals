...
/**
 * Text-material ascension_gate style multiblock:
 * radius-3 outer ring of gate blocks + four corner Spirit Ore pillars y=1..5 + clear aperture.
 */
public final class AscensionGateStructure {
    ...
    private AscensionGateStructure() {}
    ...
    public static CheckResult validate(Level level, BlockPos center, Block ringBlock, Block frameBlock) {
        ...
        return new CheckResult(missingRing, missingFrame, blocked);
    }
    ...
    public record CheckResult(int missingRingBlocks, int missingFrameBlocks, int blockedApertureBlocks) {
        public boolean complete() {
            return missingRingBlocks <= 0 && missingFrameBlocks <= 0 && blockedApertureBlocks <= 0;
        }
    }
}
