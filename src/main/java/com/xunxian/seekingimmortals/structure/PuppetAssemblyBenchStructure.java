...
/**
 * Puppet assembly bench multiblock: 3x3 base excluding center + four corner pillars y=1..2.
 * Text-material block_items_catalog id: puppet_assembly_bench.
 */
public final class PuppetAssemblyBenchStructure {
    ...
    private PuppetAssemblyBenchStructure() {}

    public static CheckResult validate(Level level, BlockPos center, Block benchBlock, Block frameBlock) {
        int missingBase = 0;
        for (BlockPos offset : BASE_OFFSETS) {
            if (!level.getBlockState(center.offset(offset)).is(benchBlock)) {
                missingBase++;
            }
        }
        int missingFrame = 0;
