...
public class YinProtectionCharmItem extends Item {
    private static final Realm MIN_REALM = Realm.QI_REFINING;
    private static final int DURATION_TICKS = 12000;

    public YinProtectionCharmItem(Properties properties) {
        super(properties);
    }
    ...
    private boolean activate(ServerPlayer player, ItemStack stack, Realm realm) {
        if (realm.ordinal() < MIN_REALM.ordinal()) {
            player.displayClientMessage(Component.translatable(
                    "message.seeking_immortals.yin_body_protection_charm.realm_too_low",
                    MIN_REALM.getDisplayName()), true);
            return false;
        }

        CompoundTag data = player.getPersistentData();
        int currentTicks = data.getInt(CatalogPillItem.YIN_PROTECTION_TICKS_KEY);
        if (currentTicks >= DURATION_TICKS) {
            player.displayClientMessage(Component.translatable(
                    "message.seeking_immortals.yin_body_protection_charm.already_active"), true);
            return false;
        }

        data.putInt(CatalogPillItem.YIN_PROTECTION_TICKS_KEY, DURATION_TICKS);
        ...
        player.displayClientMessage(Component.translatable(
                "message.seeking_immortals.yin_body_protection_charm.success"), true);
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tooltip, TooltipFlag flag) {
        tooltip.add(Component.translatable("tooltip.seeking_immortals.yin_body_protection_charm")
