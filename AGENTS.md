...
- Current version: `0.2.0` in `gradle.properties`
...
**Required:** after every code change, run `./gradlew build` and report the result. Before release/build-oriented work, update `mod_version` in `gradle.properties` using numeric `X.Y.Z` SemVer.
...
2. Classify the change before editing:
   ...
   - **Code, resources, data packs, build logic, gameplay behavior, config that ships with the mod, or generated runtime behavior**: bump `mod_version` in `gradle.properties` by one patch version (`X.Y.Z`) before the final build.
...
`ModNetwork.PROTOCOL_VERSION` is currently `23`. If packet fields/order or incompatible channel behavior change, bump the protocol version to prevent mismatched clients/servers from decoding stale packet formats.
